...
%type   <args>   arglist ne_arglist codes hashlist ne_hashlist
...
%token	<error> tERROR
%token	tIF tELSE tELSEIF tENDIF tFOR tIN tENDFOR tRETURN tYIELD tFORK tENDFORK
...
ne_hashlist:
	expr tHASHARROW expr
	    {
		Expr *e = alloc_binary(EXPR_HASHENTRY, $1, $3);
		$$ = alloc_arg_list(ARG_NORMAL, e);
	    }
	| ne_hashlist ',' expr tHASHARROW expr
	    {
		Expr *e = alloc_binary(EXPR_HASHENTRY, $3, $5);
